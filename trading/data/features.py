...
def add_session_info(df: pd.DataFrame) -> pd.DataFrame:
    """Add date, time, and session columns."""
    df = df.copy()
...
def add_vwap_deviation(df: pd.DataFrame, lookback: int = 20) -> pd.DataFrame:
    """Add VWAP deviation in standard deviation units."""
    df = df.copy()
...
def add_rsi(df: pd.DataFrame, period: int = 14) -> pd.DataFrame:
    """Compute RSI on close prices (per-day reset not needed for short period)."""
    df = df.copy()
    delta = df["close"].diff()
...
def add_atr_pct(df: pd.DataFrame) -> pd.DataFrame:
    """Add ATR as a percentage of price, and its rolling percentile."""
    df = df.copy()
    df["atr_pct"] = df["atr"] / df["close"] * 100  # ATR as % of price

    # Daily ATR_pct: use last value per day for percentile ranking
...
def add_relative_volume(df: pd.DataFrame, lookback_days: int = 5) -> pd.DataFrame:
    """Add relative volume vs same time-of-day average over past N days."""
    df = df.copy()

    # Average volume by time-of-day over lookback window
...
def add_opening_range(df: pd.DataFrame, range_minutes: int = 15) -> pd.DataFrame:
    """Add opening range high/low for each day."""
    df = df.copy()
    df["or_high"] = np.nan
    df["or_low"] = np.nan

    or_end_minute = 9 * 60 + 30 + range_minutes  # 09:30 + range
